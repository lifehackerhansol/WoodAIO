...
int main(void)
{
    ...
    patchParams[0]&=~(PATCH_DOWNLOAD_PLAY|PATCH_PROTECTION|PATCH_SOFT_RESET);
...
/*
                ...
                if( NULL != patch ) {
*/
                    dldi((byte*)__NDSHeader->arm9destination, __NDSHeader->arm9binarySize );
/*
                }
                //argv support
                if(NULL!=patch)
...
//                }
            ...
            r4BuildMaps(filename,savename); //add some dirty hack.
#endif
...
#if 0
defined(_STORAGE_r4)
            if(patchParams[0]&PATCH_SOFT_RESET){
              int f=open("fat0:/__rpg/woodreset.bin",O_RDONLY);
		if(f>=0){
			read(f,(u8*)0x027b4000,0xc000);
			close(f);
			dldi((u8*)0x027b4000,0xc000);
		}
            }
#endif
        }
