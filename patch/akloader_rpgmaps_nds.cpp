...
#include <dma4_bin.h>
void patchRomAddress(u32 addr){
	u32 *dma4=(u32*)dma4_bin;
	u32 dma4_size=((dma4_bin_size+3)&~3)/4;
	u32 i=0;
	for(;i<dma4_size;i++){
		if(dma4[i]==0xef987655)dma4[i]=addr;
	}
}
...
*/
	...
	if(fd<0||fgetFragments(fd)){
		if(fd>=0)close(fd);
		dbg_printf("nds is fragmented. Please defrag using PC. Halt.\n");while(1);
	}

	//patch save_nand_bin / sd_save.bin in dirty way
	FIL *fs=(FIL*)__get_handle(fd)->fileStruct;
	unsigned int sect=(unsigned int)(clust2sect(fs->fs,fs->sclust)<<0);
	close(fd);
	patchRomAddress(sect);
	...
	//patch save_nand_bin / sd_save.bin in dirty way
	fs=(FIL*)__get_handle(fd)->fileStruct;
	sect=(unsigned int)(clust2sect(fs->fs,fs->sclust)<<0);
	close(fd);
